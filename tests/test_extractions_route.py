...
async def test_create_extraction_persists_the_completed_run_in_the_graph(tmp_path) -> None:
    ...
    async def override_extract_and_persist_document() -> ExtractAndPersistDocument:
        return ExtractAndPersistDocument(
            ExtractDocument(
                document_store,
    ...
    app.dependency_overrides[get_extract_and_persist_document] = (
        override_extract_and_persist_document
    )
    try:
